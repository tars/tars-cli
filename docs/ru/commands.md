...
Данная команда позволяет реинициализировать TARS с новыми настройками (шаблонизатор, препроцессор). Необязательно менять эти настройки вручную, так как их можно будет изменить в интерактивном режиме. Запускает команду `gulp re-init` в TARS. **Внимание, при переинициализации все папки и файлы удаляются.**
