...
function customActions() {
    return new Promise(resolve => {
        ...
        try {
            customActionsConfig = require(dest.templater.fullPath + '/custom-update-actions.json');
        } catch (error) {
            tarsUtils.tarsSay(chalk.yellow(error.message));
            tarsUtils.tarsSay('Will continue without custom-update-actions');
            customActionsConfig = null;
            resolve();
            return;
        }
 ...
 * @param  {Error Object} error Object with info about error
 */
...
module.exports = function updateProject(options) {
    ...
    downloadNewVersion()
        .then(() => {
            ...
            if (currentTarsVersion === downloadedVersion) {
