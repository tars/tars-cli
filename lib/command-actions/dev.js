...
var devPromt = require('../promt/dev-promt');
...
function devInit(answers) {
    ...
    runCommand(path.resolve(process.env.npmRoot + '.bin/gulp'), buildOptions);
}
...
module.exports = function dev(options) {
    ...
    if (!commandOptions.silent && !commandOptions.tunnel && !commandOptions.livereload && !commandOptions.lr && !commandOptions.ie8 && !commandOptions.customFlags) {
