...
import { tarsSay, getTarsProjectVersion, getTarsConfig as currentTarsConfig } from '../../../utils';
...
module.exports = function updateBabelConfig() {
    ...
    if (babelConfig) {
        if (semver.cmp(getTarsProjectVersion(), '<', '1.6.0') && !babelConfig.presets) {
        ...
        if (semver.cmp(getTarsProjectVersion(), '>=', '1.5.0') && babelConfig.ignore &&
        ...
        if (semver.cmp(getTarsProjectVersion(), '>=', '1.10.0')) {
    ...
    if (projectPackageJson.dependencies && !projectPackageJson.dependencies['@babel/preset-env']) {
        tarsSay('Please, wait for a moment, while I\'m update Babel to 7 version...');
        tarsSay('It can take more than 1 minute.');
        ...
        tarsSay('Babel has been updated successfully!', true);
    }
};
