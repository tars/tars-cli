...
function generateBaseFiles(path, moduleName, extensions) {
    ...
    if (extensions.tpl === 'jade') {
        fs.writeFileSync(
            path + '/' + moduleName + '.' + extensions.tpl,
            'mixin ' + moduleName + '(data)'
        );
    }
}
 ...
 * @param  {String} path Path to module. Includes module name
 */
function createDataFolder(path, moduleName) {
    var processedModuleName = moduleName;
    ...
    fs.appendFileSync(path + '/data/data.js', '\n');
        ...
        if (!stats) {
            fs.mkdir(nmd, function () {
                ...
                if (opts.full) {
                    ...
                    createDataFolder(nmd, moduleName);
                }
